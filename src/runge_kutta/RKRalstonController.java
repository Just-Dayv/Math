...
public class RKRalstonController {
    @FXML
...
//                }
    ...
    public double calculateRk(double xValue, double yValue, String eqn, int i, double stepSize ){
        ...
        output+=String.format("\\hspace{1.5cm}k_2 = f(x+0.75*h,y+ 0.75*k1*h) =f(%.4f,%.4f) =  %.3f\\\\",xValue+0.75*stepSize, yValue+ 0.75*k1*stepSize,k2);

        output+=  String.format("\\hspace{1.5cm}y_{%d} = %.2f + \\frac{%.2f}{3}  + \\frac{2(%.2f)}{3} * %.2f\\\\",i+1, yValue,i*stepSize, yValue,stepSize);
