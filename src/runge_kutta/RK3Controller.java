...
//                }
    ...
    public void startKutta(ActionEvent actionEvent) {
        ...
        for (int i = 0; i < iterations + 1; i++) {
            ...
            output+="\\\\";
            output += String.format("y_{%d} = y_{%d} + \\frac {\\left(2(F_1) + 3 (F_2) +  4 (F_3)\\right)}{9}", i + 1, i);
...
//        Main.window.setScene(Main.outputScene);
    ...
    public double calculateRk(double xValue, double yValue, String eqn, int i, double stepSize ){
        ...
        output+= String.format ("F_1 = f(x,y) * stepsize = %.3f\\\\",F1);

        double F2 = stepSize * function.calculate(xValue+stepSize/2, yValue+F1/2);
        output+=String.format("F_2 = f(\\left \\frac{x+h}{2},y+ \\frac{F_1}{2}\\right)* stepsize = %.3f\\\\",F2);

        double F3 = stepSize * function.calculate(xValue+ 3*stepSize/4, yValue+3*F2/4);
        output+= String.format("F_3 = f(\\left x+\\frac{3h}{4},y+ \\frac{3F_2}{4}\\right)* stepsize = %.3f\\\\",F3);

        output+=  String.format("y_{%d} = %.2f + \\frac {\\left(2(%.2f) + 3 (%.2f) +  4 (%.2f)\\right)}{9} \n", i+1, yValue,F1, F2,F3);
