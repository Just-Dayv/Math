...
//                    stepSize.setText(newValue.replaceAll("[^\\d+(\\.)\\d*]", ""));
    ...
    public void calculate() {
        ...
        double [][] allEquationsCoeff = new double[splineValue*3][splineValue*3];
          double [][] solutionValues = new double[splineValue*3][];
...
//          }
        ...
        for (int j= 1,  i = 1, k= 1;  j < splineValue*2-1; j++)
        {

            output += String.format( "\\hspace{1.5cm} a_{%d}(%.2f)^2",k, variableValues[i]);
                output+= String.format("+ b_{%d}(%.2f) + c_{%d}= \\hspace{1.5cm}" , k,variableValues[i],k);


            output+= String.format("%.4f\\hspace{1.5cm} .....(%d)" , solutions[i], eqnCounter++);
            output+="\\\\";
            if (j%2 == 0)
            {
                i++;
        ...
        output += String.format( "\\hspace{1.5cm}a_{%d}(%.2f)^2",variableValues.length-1, variableValues[variableValues.length-1]);
        output+= String.format("+ b_{%d}(%.2f) + c_{%d}= \\hspace{1.5cm}" ,variableValues.length-1 ,variableValues[variableValues.length-1],variableValues.length-1);
        ...
        output+="\\\\";
        output+="\\\\";
        for (int i = 1;  i < variableValues.length-1; i++)
        {

            output += String.format( "\\hspace{1.5cm} 2a_{%d}(%.2f)", i, variableValues[i]);
            output+= String.format("+ b_{%d} -2a_{%d}(%.2f)- b_{%d}= \\hspace{1.5cm}0\\hspace{1.5cm} .....(%d)" , i,i+1, variableValues[i],i+1,eqnCounter++);
        ...
        output+="\\\\";
        output+="Last\\hspace{0.4cm}  Equation";
        output+="\\\\";
        output+=String.format("\\hspace{1.5cm}a_1 = 0\\hspace{1.5cm} .....(%d)", eqnCounter);
        output+="\\\\";
        ...
        double [][] differentialCoeffecients = new double[variableValues.length-2][3];
        for (int i = 0; i < differentialCoeffecients.length; i++){
            differentialCoeffecients[i][0] = 2* variableValues[i+1];
        ...
        for (int i = 0, j=0, k=0; i < splineValue* 2; i+=2,k+=1){
            ...
            Array.setDouble(allEquationsCoeff[i+1],j,constCoeff[k+1][0]);
            Array.setDouble(allEquationsCoeff[i+1],j+1,constCoeff[k+1][1]);
            Array.setDouble(allEquationsCoeff[i+1],j+2,constCoeff[k+1][2]);
            j+=3;

        }
        for (int i = splineValue*2, j=0, k=0; i < splineValue*3-1; i+=1,k+=1){
        ...
        printVarMatrix(splineValue*3);
        output+="=";

        double [] solutionCoefficients = new double[splineValue*3];

        for (int i = 1, j = 1; i <splineValue*2-1; i+=2, j+=1){
            solutionCoefficients[i] = solutions[j];
            solutionCoefficients[i+1]= solutions[j];
        }
        solutionCoefficients[0] = solutions[0];
        solutionCoefficients[splineValue*2-1] = solutions[variableValues.length-1];
        ...
        double [][] resultTable = new double[splineValue][3];
